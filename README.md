...
- **On Windows, a power cut can undo the last change to the store** — on Linux and macOS every change to the store is flushed to the disk, directory entry included, before the wallet treats it as done. Windows offers no documented way to flush the directory entry, so it is not flushed. A power cut or a system crash in the seconds after a spend can bring back the store as it was before that spend; if the spend has not yet settled on the chain, the wallet can then sign the same key position again, which is the key reuse this wallet exists to prevent. After a power cut or a crash on Windows, **wait for your last spend to settle, and run `status` before spending again**.
- **On Windows, another program can hold the store open** — an antivirus scanner, a search indexer, or a backup or sync agent. When one does, a change to the store is refused and nothing is changed; the error says so. Run the command again, and if the refusal persists, exclude the keystore directory from that program.
