...
**`create` flushes the store directory's parent.** The directory `fsync` makes the renamed snapshot's entry durable inside the store directory; the store directory's own entry lives in its parent, which none of the four steps touches. Left unflushed, a power loss on a filesystem that had not yet committed that entry could take the whole store although `create` returned and every commit inside it completed its four steps — and with it the position I2 calls durable and the reservation I3 keeps whole: a key that has signed, with nothing on disk to say so, since a restore from the phrase reads the position off the chain, where a spend that has not landed does not appear. So `create` flushes the parent once, before its first commit, whether it made the directory or found it, and refuses rather than go on if the parent cannot be opened or flushed. What that does not reach is the parent's own entry and every directory above it, which are the operator's, and whether a flush that returned has committed anything is the filesystem's to honour, as with every `fsync` here. The crash model I2 and I3 state, a kill at a syscall boundary, cannot reach any of this, because the kernel keeps what `mkdir` did.
...
What this rests on is that a flush which returned is on the device: `FlushFileBuffers` writes a file's buffered data and metadata, and the documentation gives a file just created as its example of metadata that needs it. A device that acknowledges a flush it did not perform defeats this as an `fsync` that lies defeats the Unix path. On Windows the store directory's own entry in its parent is flushed by nothing: `create` makes the call the Unix path makes to flush the parent, and on Windows that call does nothing, because Win32 documents no flush an unprivileged process can make that commits a directory's entry — `FlushFileBuffers` is documented for a file's data and metadata and, with administrative privileges, for a volume. So on Windows a power cut before the file system commits the store directory's creation can still take the whole store, with any reservation made in it meanwhile.
...
This wallet targets Unix and Windows. It is built and tested on **Linux** and **macOS**. **On Windows the board passes, and the binary has been run once, by a person**: `./board check` is green on a Windows runner, whose tests measure the keystore's access lists and its refusal of a store another program holds and drive its lock and its commit path; no test runs the shipped binary there, and its console and its generator have been exercised by one person's run at a Windows 11 console on x86-64 and by nothing else. `FORK.md` records both; `RELEASE.md` records a release's verification, and none has run on Windows. A build for any other target fails at compile time rather than degrading: `lib.rs` names the three interfaces the crate needs and `keystore` names the storage guarantees it rests on.
